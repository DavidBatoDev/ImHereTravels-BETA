...
export async function POST(req: NextRequest) {
  try {
  const { email, tourPackage, tourPackageName, amountGBP, /* bookingId (ignored to avoid re-init triggers) */ bookingId, meta } = await req.json();
    ...
    if (!existingSnap.empty) {
      ...
      try {
        const existingIntent = await stripe.paymentIntents.retrieve(data.stripeIntentId);
