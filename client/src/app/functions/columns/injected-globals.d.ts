...
declare global {
  ...
  const firebaseUtils: {
    ...
    getCollectionData: (collectionName: string, constraints?: any[]) => Promise<any[]>;
    addDocument: (collectionName: string, data: any) => Promise<string>;
    updateDocument: (collectionName: string, docId: string, data: any) => Promise<string>;
  ...
  const functionsUtils: {
    ...
    emailUtils: {
      generateReservationEmail: (bookingId: string, generateDraftCell?: boolean) => Promise<any>;
