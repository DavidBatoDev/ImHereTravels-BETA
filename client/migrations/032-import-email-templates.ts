...
const MIGRATION_ID = "032-import-email-templates";
...
function getLatestExportFile(): string {
  ...
  const files = fs
    ...
    .filter((f) => f.startsWith("email-templates-") && f.endsWith(".json"))
    .map((f) => path.join(exportsDir, f));
  if (files.length === 0) {
    throw new Error(`No email-templates-*.json export file found in ${exportsDir}. Run npm run log-email-templates first.`);
  }
...
export async function runMigration(dryRun = false) {
  ...
  const entries = Array.isArray(parsed) ? parsed : parsed.templates || parsed.data || [];
