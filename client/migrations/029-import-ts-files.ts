...
const MIGRATION_ID = "029-import-ts-files";
...
function getLatestExportFile(): string {
  ...
  const files = fs
    ...
    .filter((f) => f.startsWith("ts-files-") && f.endsWith(".json"))
    .map((f) => path.join(exportsDir, f));
  if (files.length === 0) {
    throw new Error(`No ts-files-*.json export file found in ${exportsDir}. Run npm run log-ts-files first.`);
  }
