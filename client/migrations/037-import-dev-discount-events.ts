...
const MIGRATION_ID = "037-import-dev-discount-events";
const COLLECTION_NAME = "discountEvents";
...
function getDevExportFile(): string {
  ...
  const fileName = "discountEvents-2025-11-23T16-46-52-114Z.json";
...
export async function runMigration(dryRun = false) {
  ...
  console.log(
    `📋 This migration will replace all discountEvents with dev backup data`
  );
  ...
  const entries = Array.isArray(parsed)
    ? parsed
    : parsed.discountEvents || parsed.data || [];
...
export async function rollbackMigration(): Promise<void> {
  ...
  console.log(
    `💡 Production backup location: exports/export-prod/discountEvents-*.json`
  );
}
