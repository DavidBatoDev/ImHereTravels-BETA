...
const MIGRATION_ID = "036-import-dev-tour-packages";
...
function getDevExportFile(): string {
  const exportsDir = path.join(process.cwd(), "exports", "export-dev");
  const fileName = "tourPackages-2025-11-23T15-54-42-974Z.json";
...
export async function runMigration(dryRun = false) {
  ...
  console.log(
    `📋 This migration will replace all tourPackages with dev backup data`
  );

  const filePath = getDevExportFile();
  ...
  const entries = Array.isArray(parsed)
    ? parsed
    : parsed.tourPackages || parsed.data || [];
  ...
  if (dryRun) {
    ...
    entries.forEach((entry: any, index: number) => {
      ...
      const status = entry?.data?.status || entry?.status || "unknown";
      console.log(`  ${index + 1}. ${id} - ${name} (${status})`);
    });
...
export async function rollbackMigration(): Promise<void> {
  ...
  console.log(
    `💡 Production backup location: exports/export-prod/tourPackages-2025-11-13T12-41-34-817Z.json`
  );
}
