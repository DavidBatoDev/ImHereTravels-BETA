...
const MIGRATION_ID = "028-import-ts-folders";
const COLLECTION_NAME = "ts_folders";
...
function getLatestExportFile(): string {
  ...
  const files = fs
    ...
    .filter((f) => f.startsWith("ts-folders-") && f.endsWith(".json"))
    .map((f) => path.join(exportsDir, f));
  if (files.length === 0) {
    throw new Error(`No ts-folders-*.json export file found in ${exportsDir}. Run npm run log-ts-folders first.`);
  }
