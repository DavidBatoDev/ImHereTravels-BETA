/**
 * Migration Script: Add Access Tokens to Existing Bookings (PRODUCTION)
 *
 * This script adds long-lived, secure access tokens to all existing booking documents.
 * Uses the production service account key.
 *
 * Usage:
 *   npx ts-node client/scripts/add-access-tokens-to-bookings-prod.ts --dry-run
 *   npx ts-node client/scripts/add-access-tokens-to-bookings-prod.ts --production
 */
...
async function addAccessTokensToBookings(isDryRun: boolean) {
  ...
  console.log("\n" + "=".repeat(80));
  console.log("🔐 ACCESS TOKEN MIGRATION SCRIPT (PROD)");
  console.log("=".repeat(80));
  console.log(
    `Mode: ${mode} ${isDryRun ? "(no changes will be made)" : "(will modify database)"}`,
  );
  console.log("=".repeat(80) + "\n");
  ...
  try {
    ...
    for (const doc of bookingsSnapshot.docs) {
      ...
      try {
        ...
        if (bookingData.access_token) {
          console.log(
            `${progress} ⏭️  Skipping ${bookingId} - already has access token`,
          );
        ...
        if (isDryRun) {
          console.log(
            `${progress} [DRY RUN] Would add access_token to ${bookingId}`,
          );
        ...
        console.error(
          `${progress} ❌ Error processing ${bookingId}:`,
          errorMessage,
        );
      }
    }
    ...
    console.log(
      `✅ ${isDryRun ? "Would update" : "Updated"}:           ${stats.updatedBookings}`,
    );
    console.log(
      `⏭️  Skipped:              ${stats.skippedBookings} (already had tokens)`,
    );
